function [update_stat]=write_update_time(traces, access_type,options)
% [update_stat]=write_update_time(traces, access_type,options)
% calculate the timed update ratio
% input:
%   access_type: decide if only consider read 1/write 0 or combine 2
%   options.drive_max_lba=2*1024^4/512;
%   options.access_type=0;
% output:
%   update_stat: include some fields, e.g., update_average_size, hit_ratio,
%   write_cmd_ratio, idx_write_size, write_average_size
%
...
elseif options.access_type==2 % put all togheter
    ...
    for cmd_id=1 : total_cmd
        ...
        hit_ratio(cmd_id)=hit_count; % accumulate  write update requests number
        write_ratio(cmd_id)=write_size; % accumulate  write requests' size
        update_ratio(cmd_id)=update_size; % accumulate write update requests' size (access frequency>=2)
        write_cmd_ratio(cmd_id)=write_count; % accumulate  write requests number
