from numpy import zeros, nonzero, shape, arange, transpose, dot, uint8, uint16, uint32,uint64, c_,r_,sum,squeeze, ceil,logical_and,add,copy,mod,mean,array,ones,logical_not
...
def sequential_stream_track(q_len,traces,access_type,seq_size_threshold=None):
    '''
 ...
 Author: jun.xu99@gmail.com
    ...
    print('Sequence analysis: Queue length ='+str(q_len)+' and access type ='+str(access_type))
    ...
    seq_stream_length_count_limited=zeros((max_stream_length,2),dtype=uint32)
        ...
        if logical_not(find_sequential):
            ...
            if queue_index > q_len:
                ...
                if LRU_queue[0,3] > max_stream_length:
                    seq_stream_length_count=(r_[seq_stream_length_count,zeros((LRU_queue[0,3] - max_stream_length,2),dtype=uint32)])
                    seq_stream_length_count_limited=(r_[seq_stream_length_count_limited,zeros((LRU_queue[0,3] - max_stream_length,2),dtype=uint32)])
